...
# Pre-trained word embeddings (GloVe embeddings)
# into a frozen Keras Embedding layer, and uses to
...
# GLOVE_DIR = BASE_DIR + '../xData/glove.6B/'
...
embeddings_index = {}
# f = codecs.open(os.path.join(GLOVE_DIR, 'glove.6B.100d_2.txt'))

# f = pd.read_csv(os.path.join(GLOVE_DIR, '100.txt'), sep=" ", header=None).values
f = pd.read_csv(os.path.join(GLOVE_DIR, 'glove.6B.100d_2.txt'), sep=" ", header=None).values
# f = open(os.path.join(GLOVE_DIR, 'test10.txt'))
# f_d = f.decode('utf-8')
i = 1
for line in f:
    # values_d = line.decode('utf-8')
    # values = values_d.line.split()
    # line_d = codecs.decode(line, 'utf-8')
    # values = line.split()
    # word = values[0]
